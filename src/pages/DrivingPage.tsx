...
import { Car, Navigation, Clock, MapPin, ExternalLink, Plus, Pencil, Trash2, Wand2 } from 'lucide-react';
import { format, parseISO } from 'date-fns';
import { useTripContext } from '../context/TripContext';
import { saveDrivingSegment, deleteDrivingSegment, saveRentalCar, deleteRentalCar, autoGenerateDrivingRoutes } from '../firebase/tripService';
...
const DrivingPage: React.FC = () => {
  ...
  const { driving, rentalCars, days, tripCode, isAdmin, hotels, flights, config } = useTripContext();
  ...
  async function handleAutoplan() {
    ...
    try {
      const count = await autoGenerateDrivingRoutes(tripCode, hotels, flights, driving, config?.startDate);
      alert(count > 0
        ? (isHe ? `נוצרו ${count} מסלולים חדשים בהצלחה!` : `Created ${count} new route segments!`)
        : (isHe ? 'לא נמצאו מסלולים חדשים (כולם כבר קיימים)' : 'No new routes to generate (all already exist)'));
    } catch (err) {
      console.error('Auto-plan failed:', err);
      alert(isHe ? 'שגיאה ביצירת מסלולים' : 'Failed to generate routes');
    } finally {
  ...
  return (
    <div className="driving-page">
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
        <h1 className="page-title" style={{ margin: 0 }}>{t('driving.title')}</h1>
        {isAdmin && (
          <button
          ...
          >
            ...
            {autoPlanning
              ? (isHe ? 'יוצר...' : 'Planning...')
              : (isHe ? 'תכנון אוטומטי' : 'Auto-plan routes')
            }
          </button>
        )}
      </div>
