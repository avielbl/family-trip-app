...
import { estimateRouteByGeo } from '../utils/geocode';
import type {
  TripConfig,
  TripDay,
  Flight,
...
async function estimateRoute(from: string, to: string): Promise<{ distanceKm: number; durationMinutes: number }> {
  ...
  // Any destination: geocode both endpoints and estimate by road distance.
  const geo = await estimateRouteByGeo(from, to);
  if (geo) return geo;
  return { distanceKm: 150, durationMinutes: 120 }; // last-resort fallback
}
...
export async function autoGenerateDrivingRoutes(
  ...
  if (sortedFlights.length > 0) {
    ...
    if (!alreadyExists) {
      const est = await estimateRoute(arrivalCity, firstHotel.city);
      segments.push({
  ...
  for (let i = 0; i < sortedHotels.length - 1; i++) {
    ...
    if (!alreadyExists) {
      ...
      const est = await estimateRoute(h1.city, h2.city);
      segments.push({
  ...
  if (sortedFlights.length > 0) {
    ...
    if (!alreadyExists) {
      const est = await estimateRoute(lastHotel.city, departureCity);
      segments.push({
